<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-semibold text-xl text-gray-900">Edit Eksemplar Buku</h2>
                <p class="mt-1 text-sm text-gray-500">Update data kondisi fisik dan status eksemplar buku.</p>
            </div>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-3xl border border-green-100 overflow-hidden">
                <div class="bg-gradient-to-r from-emerald-500 to-teal-500 p-6">
                    <h3 class="text-white text-lg font-semibold">Form Edit Eksemplar Buku</h3>
                    <p class="text-emerald-100 text-sm mt-1">Perbarui informasi kondisi dan status copy fisik buku.</p>
                </div>
                <div class="p-6 space-y-6">
                    <form method="POST" action="{{ route('book_items.update', $bookItem) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                                ...
                                <div class="relative mt-2">
                                    <select id="book_id" name="book_id" required style="appearance: none; -webkit-appearance: none; -moz-appearance: none;" class="appearance-none block w-full rounded-2xl border border-green-200 bg-green-50 px-4 py-3 pr-10 text-gray-900 shadow-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
                                        ...
                                        @forelse($books as $book)
                                            <option value="{{ $book->id }}" {{ old('book_id', $bookItem->book_id) == $book->id ? 'selected' : '' }}>{{ $book->title }}</option>
                                        @empty
                            ...
                            <div>
                                ...
                                <input id="item_code" name="item_code" type="text" value="{{ old('item_code', $bookItem->item_code) }}" placeholder="Contoh: BK001" required class="mt-2 block w-full rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                            ...
                            <div>
                                ...
                                <div class="relative mt-2">
                                    <select id="condition" name="condition" required style="appearance: none; -webkit-appearance: none; -moz-appearance: none;" class="appearance-none block w-full rounded-2xl border border-green-200 bg-green-50 px-4 py-3 pr-10 text-gray-900 shadow-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
                                        ...
                                        <option value="cukup" {{ old('condition', $bookItem->condition) == 'cukup' ? 'selected' : '' }}>Cukup</option>
                                        <option value="rusak" {{ old('condition', $bookItem->condition) == 'rusak' ? 'selected' : '' }}>Rusak</option>
                                    </select>
                                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-4 text-green-500">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                    </div>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                            ...
                            <button type="submit" class="inline-flex justify-center rounded-full bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-300">Perbarui Eksemplar</button>
                        </div>
                    </form>
                </div>
            </div>
