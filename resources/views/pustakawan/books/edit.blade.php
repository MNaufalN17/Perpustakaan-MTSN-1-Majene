<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-semibold text-xl text-gray-900">Edit Buku</h2>
                <p class="mt-1 text-sm text-gray-500">Perbarui data buku untuk memastikan katalog tetap akurat.</p>
            </div>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-3xl border border-green-100 overflow-hidden">
                <div class="bg-gradient-to-r from-emerald-500 to-teal-500 p-6">
                    <h3 class="text-white text-lg font-semibold">Form Ubah Buku</h3>
                    <p class="text-emerald-100 text-sm mt-1">Perbarui informasi buku yang diperlukan pada katalog perpustakaan.</p>
                </div>
                    ...
                    <form method="POST" action="{{ route('books.update', $book) }}" class="space-y-6">
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input id="title" name="title" type="text" value="{{ old('title', $book->title) }}" required class="mt-2 block w-full rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                                @error('title')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                            </div>

                            <div>
                                <label for="author" class="block text-sm font-medium text-gray-700">Penulis</label>
                                <input id="author" name="author" type="text" value="{{ old('author', $book->author) }}" required class="mt-2 block w-full rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input id="publisher" name="publisher" type="text" value="{{ old('publisher', $book->publisher) }}" required class="mt-2 block w-full rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                                ...
                                <div class="relative mt-2">
                                    <select id="ddc_class_id" name="ddc_class_id" required style="appearance: none; -webkit-appearance: none; -moz-appearance: none;" class="appearance-none block w-full rounded-2xl border border-green-200 bg-green-50 px-4 py-3 pr-10 text-gray-900 shadow-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
                                        ...
                                        @forelse($ddcClasses as $ddcClass)
                                            <option value="{{ $ddcClass->id }}" {{ old('ddc_class_id', $book->ddc_class_id) == $ddcClass->id ? 'selected' : '' }}>{{ $ddcClass->code }}</option>
                                        @empty
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                            ...
                            <button type="submit" class="inline-flex justify-center rounded-full bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-300">Perbarui Buku</button>
                        </div>
                    </form>
                </div>
            </div>
