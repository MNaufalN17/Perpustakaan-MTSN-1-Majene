<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-semibold text-xl text-gray-900">Edit Anggota</h2>
                <p class="mt-1 text-sm text-gray-500">Perbarui informasi anggota sesuai data terbaru.</p>
            </div>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-3xl border border-emerald-100 overflow-hidden">
                <div class="bg-gradient-to-r from-emerald-500 to-teal-500 p-6">
                    <h3 class="text-white text-lg font-semibold">Form Ubah Anggota</h3>
                    <p class="text-emerald-100 text-sm mt-1">Edit profil anggota agar data perpustakaan tetap akurat.</p>
                </div>
                    ...
                    <form method="POST" action="{{ route('members.update', $member) }}" class="space-y-6">
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input id="member_code" name="member_code" type="text" value="{{ old('member_code', $member->member_code) }}" required class="mt-2 block w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                            ...
                            <div>
                                ...
                                <input id="nis_nip" name="nis_nip" type="text" value="{{ old('nis_nip', $member->nis_nip) }}" required class="mt-2 block w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input id="name" name="name" type="text" value="{{ old('name', $member->name) }}" required class="mt-2 block w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                                @error('name')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                            </div>

                            <div>
                                <label for="phone" class="block text-sm font-medium text-gray-700">No. HP</label>
                                <input id="phone" name="phone" type="text" value="{{ old('phone', $member->phone) }}" class="mt-2 block w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                            ...
                            <div>
                                ...
                                <div class="relative mt-2">
                                    <select id="member_type" name="member_type" required class="appearance-none block w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 pr-10 text-gray-900 shadow-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
                                        ...
                                        <option value="siswa" {{ old('member_type', $member->member_type) == 'siswa' ? 'selected' : '' }}>Siswa</option>
                                ...
                                <div class="relative mt-2">
                                    <select id="student_class_id" name="student_class_id" class="appearance-none block w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 pr-10 text-gray-900 shadow-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
                                        ...
                                        @forelse($classes as $class)
                                            <option value="{{ $class->id }}" {{ old('student_class_id', $member->student_class_id) == $class->id ? 'selected' : '' }}>{{ $class->class_name }}</option>
                                        @empty
                        ...
                        <div>
                            ...
                            <input id="card_image" name="card_image" type="text" value="{{ old('card_image', $member->card_image) }}" placeholder="Path atau nama file kartu" class="mt-2 block w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                            ...
                            <button type="submit" class="inline-flex justify-center rounded-full bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-300">Perbarui Anggota</button>
                        </div>
                    </form>
                </div>
            </div>
